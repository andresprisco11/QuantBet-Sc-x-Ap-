...
misma señal de goles) mueve la aguja. Comparacion directa contra v2: mismo
conjunto de 1,900 partidos.
...
def run():
    ...
    print("\n=== Resultados FUERA DE MUESTRA v4 (recencia + PROMOTED_TEAM + forma reciente por tiros) ===")
    ...
    print("\n(Referencia v2 -- mismo conjunto de partidos, comparacion directa valida: "
          "modelo 0.598631, mercado 0.560801, blend 0.569562, gap +0.008761)")

    out_path = PROCESSED_DATA_DIR / "EPL" / "model_predictions_oos_walkforward_v4.csv"
    ...
    log_run(
        ...
        features="goals ~ is_home + C(team) + C(opponent) + team_recent_form [+ filas sinteticas PROMOTED_TEAM], "
        ...
        notes="Walk-forward v4: primera variable de feature engineering (forma reciente por tiros al arco) "
              "encima de v2 (recencia + PROMOTED_TEAM). No incluye Dixon-Coles (v3, resultado negativo documentado).",
    )
