...
ganadora de ESTE MISMO barrido sobre el MISMO conjunto de 1,730 partidos
es, en sí mismo, un ejercicio de ajustar un hiperparámetro sobre datos ya
...
más datos futuros (temporada 2627 en curso, o paper trading), no como un
...
def run():
    ...
    print("=== Barrido completo de reglas de staking (ordenado por bankroll final, mejor primero) ===")
    ...
    print(f"\n=== Mismo barrido, filtrado a combinaciones con al menos {MIN_N_BETS_FOR_TRUST} apuestas "
          f"(las demas son muestra demasiado chica para confiar) ===")
    print(robust.round(4).to_string(index=False))

    out_path = Path(__file__).resolve().parent.parent.parent / "data" / "runs" / "staking_sweep_v4.csv"
    ...
    print("\nNo se loggea en el sistema de tracking (son 24 corridas de un mismo analisis, no 24 modelos "
          "distintos) -- una vez elegida la combinacion final, esa SI se corre con economic_backtest.py "
          "(actualizando sus constantes MIN_EDGE_THRESHOLD/KELLY_FRACTION/MAX_ODDS) y esa corrida queda "
          "trazada normalmente.")
